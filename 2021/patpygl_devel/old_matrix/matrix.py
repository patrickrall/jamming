...
class NPMat():
    ...
    def inverse(self):
        out = NPMat()
        out.array = np.linalg.inv(self.array)
        return out

    @property
    def buffer(self):
        r,c = self.array.shape
        buf = (_dtypes["float32"]*(r*c))(0)
        for i in range(r):
            for j in range(c):
                buf[i*c+j] = self.array[i,j]
        return buf
...
def np_zeros(r,c):
    out = NPMat()
    out.array = np.zeros([r,c],dtype=float)
    return out

def np_eye(size):
    out = NPMat()
    out.array = np.eye(size,dtype=float)
    return out
