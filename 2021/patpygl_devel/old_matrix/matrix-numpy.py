...
# An attempt at making my matrix library have numpy support.
# Ran out of morale - too cumbersome.
...
class Vec():
    def __init__(self,*args,dtype='float32'):
        ...
        assert len(dat) > 0
        self.array = np.array(dat, dtype=_np_dtypes[dtype])

    @property
    def buffer(self):
        ctypes_type = ctypes.POINTER(_c_dtypes[dtype]*len(dat))
        return self.array.ctypes.data_as(ctypes_type).contents

    @property
    def size(self):
        return ctypes.sizeof(_c_dtypes[self._dtype])*len(self)
    ...
    def __setitem__(self,key,value):
        ...
        if 'bool' in self.dtype: self.array[key] = (float(value) != 0.)
        if 'int' in self.dtype: self.array[key] = int(value)
        if 'float' in self.dtype: self.array[key] = float(value)
    ...
    ################## THIS IS WHERE I RAN OUT OF MORALE. CONTINUE IMPLEMENTING BELOW!
